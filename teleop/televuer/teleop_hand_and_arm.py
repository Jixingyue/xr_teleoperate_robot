...
if __name__ == '__main__':
    ...
    parser.add_argument('--arm', type=str, choices=['G1_29', 'G1_23', 'H1_2', 'H1'], default='G1_29', help='选择手臂控制器')
    ...
    parser.add_argument('--headless', action='store_true', help='启用无界面模式（不显示画面）')
    parser.add_argument('--sim', action = 'store_true', help = '启用 isaac 仿真模式')

    args = parser.parse_args()
    logger_mp.info(f"args: {args}")

    # 图像客户端：img_config 应与 image_server.py（机器人开发计算单元上的）中的配置保持一致
    if args.sim:
        img_config = {
            'fps': 30,
    ...
    ASPECT_RATIO_THRESHOLD = 2.0 # 宽高比超过该值即视为双目
    ...
    # television（遥视觉）：从 XR 设备获取手部位姿数据，并将机器人的头部相机图像传输到 XR 设备。
    ...
    if args.ee == "dex3":
        ...
        dual_hand_state_array = Array('d', 14, lock = False)   # [输出] 当前左右手状态(14)数据。
        dual_hand_action_array = Array('d', 14, lock = False)  # [输出] 当前左右手动作(14)数据。
        ...
        dual_gripper_state_array = Array('d', 2, lock=False)   # 当前左右夹爪状态(2)数据。
        dual_gripper_action_array = Array('d', 2, lock=False)  # 当前左右夹爪动作(2)数据。
        ...
        right_hand_pos_array = Array('d', 75, lock = True)     # [输入]
        dual_hand_data_lock = Lock()
        dual_hand_state_array = Array('d', 12, lock = False)   # [输出] 当前左右手状态(12)数据。
        dual_hand_action_array = Array('d', 12, lock = False)  # [输出] 当前左右手动作(12)数据。
        ...
        while running:
            ...
            if args.record:
                ...
                if is_recording:
                    ...
                    states = {
                        "left_arm": {                                                                    
                            "qpos":   left_arm_state.tolist(),    # numpy.array 转 list
                            "qvel":   [],                          
                            "torque": [],                        
                        }, 
                        "right_arm": {                                                                    
